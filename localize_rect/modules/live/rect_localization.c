...
#define MAX_RECT_LOCALIZATION_CALLBACKS 20
...
static pthread_mutex_t      rect_localization_lock;
static int                  fd[2];

static corners_data         corners_local_copy;
static base_data_type       base_data_local_copy;

static rect_localization_t  localization_data_local;

static rect_localization_receive_data_callback  callbacks[MAX_RECT_LOCALIZATION_CALLBACKS];
...
static point_2d right_top_corner = {
  .x = RECT_MAP_WITH_IN_MM,
  .y = RECT_MAP_HEIGHT_IN_MM
};
...
void get_map_x_and_y(int *corner, double *difference_x, double *difference_y, double *x, double *y)
{
  switch (*corner) {
    case RIGHT_TOP_CORNER:
      *x = RECT_MAP_WITH_IN_MM - *difference_x;
      *y = RECT_MAP_HEIGHT_IN_MM - *difference_y;
      break;
    case RIGHT_BOTTOM_CORNER:
      *x = RECT_MAP_WITH_IN_MM - *difference_x;
      *y = *difference_y;
...
void *rect_localization_thread(void *args)
{
  while (program_runs)
  {
    if (wait_for_new_data(fd) < 0) {
      perror("mikes:rect_localization");
      mikes_log(ML_ERR, "rect_localization error during waiting on new Data.");
      continue;
    }
    get_base_data(&base_data_local_copy);
    pthread_mutex_lock(&rect_localization_lock);
    process_all_data();
    pthread_mutex_unlock(&rect_localization_lock);
  }

  mikes_log(ML_INFO, "rect_localization quits.");
...
{
  if (pthread_mutex_trylock(&rect_localization_lock) == 0)
  {
...
{
  if (!mikes_config.use_rect_localization)
  {
    mikes_log(ML_INFO, "rect_localization supressed by config.");
    online = 0;
    return;
  }
  online = 1;
  ...
  {
    perror("mikes:rect_localization");
    mikes_log(ML_ERR, "creating pipe for rect localization");
    return;
  }

  pthread_t t;
  pthread_mutex_init(&rect_localization_lock, 0);
  register_tim_corner_callback(tim_corner_new_data);
  if (pthread_create(&t, 0, rect_localization_thread, 0) != 0)
  {
    perror("mikes:rect_localization");
    mikes_log(ML_ERR, "creating thread for rect localization");
  }
...
void register_rect_localization_callback(rect_localization_receive_data_callback callback)
{
  ...
  if (callbacks_count >= MAX_RECT_LOCALIZATION_CALLBACKS)
  {
     mikes_log(ML_ERR, "too many rect_localization callbacks");
     return;
  }
